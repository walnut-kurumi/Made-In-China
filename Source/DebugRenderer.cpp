...
DebugRenderer::DebugRenderer(ID3D11Device* device)
{
	// 頂点シェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, ".\\shader\\obj\\debug_vs.cso", "rb");
	...
	// ピクセルシェーダー
	{
		// ファイルを開く
		FILE* fp = nullptr;
		fopen_s(&fp, ".\\shader\\obj\\debug_ps.cso", "rb");
		_ASSERT_EXPR_A(fp, "CSO File not found");
...
// 描画開始
void DebugRenderer::Render(ID3D11DeviceContext* context, const DirectX::XMFLOAT4X4& view_projection)
{
	// シェーダー設定
